...
pub fn initialize_snapshot() -> Snapshot<15, 6, 2, 8> {
    ...
    let csv_entry_path: String = Input::with_theme(&ColorfulTheme::default())
        .with_prompt("Enter path to entry CSV file")
        .with_initial_text("csv/two_assets_entry_2_15.csv")
        .interact_text()
        .unwrap();

    let csv_signature_path: String = Input::with_theme(&ColorfulTheme::default())
        .with_prompt("Enter path to signature CSV file")
        .with_initial_text("csv/signatures.csv")
        .interact_text()
        .unwrap();

    let ptau_path: String = Input::with_theme(&ColorfulTheme::default())
...
pub async fn initialize_client() -> SignerMiddleware<Provider<Http>, LocalWallet> {
    ...
    let provider_url: String = Input::with_theme(&ColorfulTheme::default())
